...
function findAnagrams(s, p) {
  const result = [];
  const charCount = new Array(26).fill(0); // Array to store the count of characters in p
  ...
  while (right < s.length) {
    ...
    if (charCount[s.charCodeAt(right) - "a".charCodeAt(0)] > 0) {
      count--;
    }

    charCount[s.charCodeAt(right) - "a".charCodeAt(0)]--; // Decrement the count for the current character
    ...
    if (right - left === p.length) {
      ...
      charCount[s.charCodeAt(left) - "a".charCodeAt(0)]++; // Increment the count for the excluded character
